...
library(tidyr)
library(dplyr)
file <- read.delim2("bulkRNA_PS19_P0/refs/fastq_headers.tsv", sep = "\t", header = FALSE)
...
# PP_1_F_1.fq.gz
file$mouse <- str_match(file$fastq_file, "(PP_[0-9]+)_[FI]_[12].fq.gz")[,2]
file$group <- str_match(file$fastq_file, "PP_[0-9]+_([FI]+)_[12].fq.gz")[,2]
file$group <- factor(file$group, levels = c("I","F"))
file$read <- str_match(file$fastq_file, "PP_[0-9]+_[IF]+_([12]).fq.gz")[,2]
